...
def get_environment(config: Config) -> BaseEnv:
    ...
    if os.name == 'nt':
        env = UnityEnvironment(file_name='Build/GridWorld.exe', side_channels=[channel])
...
def run_agent(config: Config):
    ...
    if bool(config.RuntimeArgs.save_stats):
    ...
    while True:
        ...
        if bool(chkpt_save) and episode % chkpt_save == 0:
            save_checkpoint(agent, episode, 
                            eps, eps_end, eps_decay,
                            os.path.join(config.RuntimeArgs.save_checkpoint, f"checkpoint_ep{episode}.tar"))
        # Any stats to save?
        if bool(stats_save) and episode % stats_save == 0 and len(running_avg_reward) == maxlen:
